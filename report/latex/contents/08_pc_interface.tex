...
In \code{piano\_key\_monitor.py}, serial reception runs in a background thread so the Tkinter GUI does not block while waiting for UART bytes. The reader thread accepts only lines matching the \code{KEY,75,...,45,...} format and pushes parsed events into a queue. The GUI periodically polls this queue, updates the highlighted keys, and calls the audio backend before redrawing the canvas so note start and stop are not delayed by screen updates.

\placeholderfigure{Insert a screenshot of the Python piano GUI showing highlighted keys.}{PC piano GUI receiving FPGA-computed key states.}{fig:piano_gui}{figures/piano_gui_update.png}
