...
\begin{table}[H]
    ...
    \begin{tabular}{c c c c c c}
        ...
        \midrule
        $k_{\mathrm{global}}$ & 10 & 11 & 12 & 13 & 14 \\
        \bottomrule
    \end{tabular}
\end{table}
...
The module \code{platform\_tracker\_controller.sv} latches the latest 75 Hz and 45 Hz classifier results separately. A movement decision is evaluated only after both fingers have produced fresh classifications, so the tracker does not combine a new key from one finger with an old key from the other. The local key IDs from the classifier are converted to signed local indices by subtracting the local center ID 2, giving the range $-2$ to $+2$.

Before issuing a move, the controller checks several blocking conditions: tracking must be enabled, both signal strengths must exceed the minimum threshold, the 75 Hz finger must not appear to the right of the 45 Hz finger, the stepper must not already be busy, the cooldown counter must be zero, and the platform center must remain within keys 2 through 12. If the requested direction passes these checks for \code{STABLE\_COUNT}=3 consecutive fresh decisions, the tracker emits one \code{move\_pulse}, sets the direction command, and updates the stored platform center by one key. The separate \code{stepper\_drv8825\_controller.sv} module then converts that pulse into 100 physical STEP pulses.
